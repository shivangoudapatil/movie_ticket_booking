...
public class SingleDatabase {
    ...
    SingleDatabase() {
        try {
            ...
            c = DriverManager.getConnection("jdbc:postgresql://localhost:5432/moviedb","postgres","11111");
    ...
    void showReviews(DefaultTableModel model) {
        try {
            String query = "select * from ratedmovies;";
            ResultSet rs = stm.executeQuery(query);
            ResultSetMetaData rsmd = rs.getMetaData();
            int cols = rsmd.getColumnCount();
            String[] colname = {"Movie","Rating", "Review"};
